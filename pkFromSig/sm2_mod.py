...
class CryptSM2(object):
    ...
    def getPK(self, R, S, data: bytes) -> None:
        """
        sig = (r, s), data = e
        """
        pk = [0, 0]
        ...
        # (s + r) ^{-1} mod n
        r_1 = pow(S + R, int(self.ecc_table['n'], base=16) - 2, int(self.ecc_table['n'], base=16))  # 求逆

        # kG
        r_2 = []
        x = ( R - int(data.hex(), 16) ) % int(self.ecc_table['n'], 16)
        Dy = pow(x, 3, int(self.ecc_table['p'], 16)) \
            + (int(self.ecc_table['a'], 16) * x ) % int(self.ecc_table['p'], 16) \
            + int(self.ecc_table['b'], 16)
        Dy %= int(self.ecc_table['p'], 16)
        y = modular_sqrt(Dy, int(self.ecc_table['p'], 16))
        r_2.append(hex(x)[2:].rjust(64, '0') + hex(y)[2:].rjust(64, '0'))

        y = (int(self.ecc_table['p'], 16) - y) % int(self.ecc_table['p'], 16)
        r_2.append(hex(x)[2:].rjust(64, '0') + hex(y)[2:].rjust(64, '0'))

        # -sG
        r_3 = neg_point(self.ecc_table['g'])
        r_3 = self._kg(S, r_3)  # -sG
        # kG - sG
        r_4 = [0, 0]
        r_4[0] = ecc_add(r_2[0], r_3)
        pk[0] = self._kg(r_1, r_4[0])  # 64 byte

        r_4[1] = ecc_add(r_2[1], r_3)
        pk[1] = self._kg(r_1, r_4[1])  # 64 byte
    ...
    def sign(self, data, K):  # 签名函数, data消息的hash，private_key私钥，K随机数，均为16进制字符串
        ...
        d_1 = pow(d+1, int(self.ecc_table['n'], base=16) - 2, int(self.ecc_table['n'], base=16))  # 求逆
