...
void cppSM4::encrypt(byte* textOut_ptr) {
	...
	// 8次循环展开
	for (int round = 0; round < SM4_ROUND;) {
...
void cppSM4::generateKey() {
	...
	for (int i = 0; i < 4; i++) {
		rk_queue[i] = _byteswap_ulong(rk_queue[i]);
		rk_queue[i] ^= FK[i];
	}
	// 生成轮密钥
...
byte cppSM4::sbox(byte& input) {
	byte row = input >> 4;  // 前4 bit - row
	byte col = input % 0x10;  // col
...
void cppSM4::unrollLoop() {
	loop_unrolling = true;
}


// --------------------- SIMD ----------------------
void cppSM4::Qinit_SIMD() {
	/// <summary>
	/// res -> X0, X1, X2, X3
	/// rk ->  k0, k1, ..., k31
	/// </summary>
	...
	// 初始化SIMD寄存器
	block reg_mem[4][8] = {
		{ textIn_ptr[0], textIn_ptr[4], textIn_ptr[8], textIn_ptr[12], textIn_ptr[16], textIn_ptr[20], textIn_ptr[24], textIn_ptr[28] },
		{ textIn_ptr[1], textIn_ptr[5], textIn_ptr[9], textIn_ptr[13], textIn_ptr[17], textIn_ptr[21], textIn_ptr[25], textIn_ptr[29] },
		{ textIn_ptr[2], textIn_ptr[6], textIn_ptr[10], textIn_ptr[14],textIn_ptr[18], textIn_ptr[22], textIn_ptr[26], textIn_ptr[30] },
		{ textIn_ptr[3], textIn_ptr[7], textIn_ptr[11], textIn_ptr[15],textIn_ptr[19], textIn_ptr[23], textIn_ptr[27], textIn_ptr[31] }
	};
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 8; j++)
			reg_mem[i][j] = _byteswap_ulong(reg_mem[i][j]);  // 转化为大端存储
	}
	res_simd = new __m256i[4];
	for (int i = 0; i < 4; i++)
		res_simd[i] = _mm256_loadu_si256((__m256i const*)(&reg_mem[i]));
}


void cppSM4::Qencrypt_SIMD(byte* textOut_ptr) {
	/// <summary>
	/// 处理len(textIn) == 8 * 128
	/// </summary>
	...
	for (int round = 0; round < SM4_ROUND;) {
		// 生成下一 Xi
		new_block = F_SIMD(rk[round++]);

		// 更新res
		res_simd[0] = res_simd[1], res_simd[1] = res_simd[2];
		res_simd[2] = res_simd[3], res_simd[3] = new_block;
	}

	// [反序变换]
	swap(res_simd[0], res_simd[3]), swap(res_simd[1], res_simd[2]);
	
	block* buffer = new block [8];
	for (int i = 0; i < 4; i++) {
		// 取回数据
		_mm256_storeu_si256((__m256i*)buffer, res_simd[i]);
		
		for (int j = 0; j < 8; j++) {
			// 转回小端并输出
			((block*)textOut_ptr)[i + 4 * j] = _byteswap_ulong(buffer[j]);
		}
	}
	delete[] buffer;
}
...
__m256i cppSM4::F_SIMD(block& cur_rk) {
	block Qcur_rk[4] = { cur_rk, cur_rk, cur_rk, cur_rk };
	__m256i cur_rk_simd = _mm256_loadu_si256((__m256i const*)Qcur_rk);

	__m256i input_T_simd = _mm256_xor_si256(_mm256_xor_si256(res_simd[1], res_simd[2]),
									   _mm256_xor_si256(res_simd[3], cur_rk_simd));

	// s-box查表顺序计算
	block res_T[8] = { 0x00 };
	_mm256_storeu_si256((__m256i*)res_T, input_T_simd);

	for (int i = 0; i < 8; i++) {
		res_T[i] = T_opt(res_T[i]);
	}

	__m256i res_T_simd = _mm256_loadu_si256((__m256i const*)res_T);
	return _mm256_xor_si256(res_simd[0], res_T_simd);
}
